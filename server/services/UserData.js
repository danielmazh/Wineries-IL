...
async function userData(storedData) {
  ...
  // Generate conditions for time ranges and the selected day
  const conditions = [];
  let conditionParamIndex = 3; // start the index at 3 because the first two queryParams are already in use
  for (const timeRange of timeRanges) {
    if (storedData[timeRange.name]) {
      conditions.push(`($${conditionParamIndex} BETWEEN substring(oh_${day} from 1 for 5) AND substring(oh_${day} from 7 for 5))`);
      queryParams.push(timeRange.range[0]);
      conditionParamIndex++;
    }
  }
