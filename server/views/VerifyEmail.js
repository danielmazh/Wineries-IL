...
function verifyEmail(req, res, next) {
  ...
  const lastRequestTime = tokenRequestTimeStore[token];
  const currentTime = Date.now();

  if (lastRequestTime && currentTime - lastRequestTime < 3000) {
    // The last request for this token was within 3 seconds, send a success message
    res.send(`
      <p>המייל אומת בהצלחה, <a href="https://www.wineries-il.co.il/">חזרה להתחברות</a></p>
    `);
    return;
  ...
  // Update the request time for this token in the store
  tokenRequestTimeStore[token] = currentTime;
